...
class _AddNewRoomState extends State<AddNewRoom> {
 ...
 Future getroomsall() async {
   ...
   roomstype.clear();
    ...
    if (response.statusCode == 200) {
     ...
     for(int i =0; i<decoded['response'].length; i++){
       ...
       roomnum.add(decoded['response'][i]['name']);
  ...
  Future addRoomNew (String id) async {
    ...
    final String apiUrl = "https://core-graduation.herokuapp.com/addRoomToDepartment?idDep=$id&number=${_number.text}&type=$type&campous=$camp&name=$name";
    print(apiUrl);
    final response =
        await http.get(Uri.parse(apiUrl));
    if (response.statusCode == 200) {
            ...
            children: [
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: Text("اسم القاعة ",style: GoogleFonts.amiri(
                                      fontSize: 18,
                                      color:Colors.grey[600],
           ...
           child: Container(
             ...
             child: InkWell(
                        onTap: ()async {
                          ...
                          else if(_value2 == 4 && _name.text.isEmpty){
                            Dialog alert = showAlert(context,'يجب ادخال جميع البيانات المطلوبة',1);
                          showDialog(
                            context: context,
                           child:alert,
